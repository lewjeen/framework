...
public class ByteBuffer extends MinaObject {
	...
	public void appendCString(String string) {
		try {
			appendString0(string, true, MinaConstant.ENC_ISO8859_1);
	...
	/*
	 * public void appendString(String string) { try { appendString(string,
	 * SmsConstant.ENC_ISO8859_1); } catch (UnsupportedEncodingException e) { //
	 * this can't happen as we use ASCII encoding // whatever is in the buffer
	 * it gets interpreted as ascii } }
	 */
	...
	public void appendString(String string, int count) {
		try {
			appendString(string, count, MinaConstant.ENC_ISO8859_1);
		} catch (UnsupportedEncodingException e) {
			// this can't happen as we use ASCII encoding
			// whatever is in the buffer it gets interpreted as ascii
		}
	...
	public String removeStringEx(int size)
			throws NotEnoughDataInByteBufferException {
		int len = length();
		int zeroPos = 0;
		if (len < size) {
	...
	public ByteBuffer readBytes(int count)
			throws NotEnoughDataInByteBufferException {
		int len = length();
		System.out.println("=============readBytes=====len======" + len);
		System.out.println("=============readBytes====count=======" + count);
