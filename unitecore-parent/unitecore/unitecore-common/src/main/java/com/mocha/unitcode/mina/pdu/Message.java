 ...
 * @param <E>
 */
public class Message<E> extends ByteData implements Serializable {
	/**
	 * 单条类型
	 */
	public byte TODO_TYPE_SINGLE = 0;
	/**
	 * 批量类型
	 */
	public byte TODO_TYPE_BATCH = 1;
	/**
	 * 字符集 -UTF-8
	 */
	public byte FORMAT_UTF8 = 10;

	/**
	 * 字符集 -UTF-8
	 */
	public byte FORMAT_GBK = 15;
	...
	/**
	 * 
	 * 设置待办数据 默认为utf-8 字符 单条发送
	 * 
	 * @param e
	 *            待办数据实体
	 */
	public void setEntityMessage(E e) {
		this.setEntityMessage(e, FORMAT_UTF8, TODO_TYPE_SINGLE);
	}
	 ...
	 * 
	 * @param e
	 *            待办实体泛型 待办数据实体 或待办数据集合
	 * @param type
	 *            单条和批量处理参数 0--单条 1--批量
	 */
	public void setEntityMessage(E e, byte type) {
		this.setEntityMessage(e, FORMAT_UTF8, type);
	}
	...
	public void setEntityMessage(E e, byte msgFormat, byte type) {
		if (type == TODO_TYPE_SINGLE) {
			...
			List<String> list = ListUtil.ToObjectString((List) e);
			this.messageData = JSON.toJSONString(list).getBytes();
		}
		this.type = type;
		this.msgFormat = msgFormat;
		this.clazz = ObjectUtil.getListClass(e).getCanonicalName().getBytes();
		// this.crc16Code = Crc16Test.toByteArray(e);
		setMsgFormat(msgFormat);
	}
	...
	public E getTodoEntity() {
		...
		try {
			...
			// + str);
			if (type == TODO_TYPE_SINGLE) {
				todo = (E) JSON.parseObject(str, ObjectUtil.getClass(Class
						.forName(new String(clazz)).newInstance()));
			} else if (type == TODO_TYPE_BATCH) {
				List<String> list = JSON.parseObject(str, List.class);
				todo = (E) ListUtil.ToStringObject(list, ObjectUtil
						.getClass(Class.forName(new String(clazz))
								.newInstance()));
			}
