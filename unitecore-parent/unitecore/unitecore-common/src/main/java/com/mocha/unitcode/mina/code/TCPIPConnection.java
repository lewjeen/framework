...
public class TCPIPConnection extends Connection {
	...
	public TCPIPConnection(int port) {
		if ((port >= MinaConstant.MIN_VALUE_PORT)
				&& (port <= MinaConstant.MAX_VALUE_PORT)) {
	...
	public TCPIPConnection(String listenIp, int listenPort) {
		if (listenIp.length() >= MinaConstant.MIN_LENGTH_ADDRESS) {
			this.remoteAddress = listenIp;
		} else {
			logger.error("Invalid remoteAddress.");
		}
		if ((listenPort >= MinaConstant.MIN_VALUE_PORT)
				&& (listenPort <= MinaConstant.MAX_VALUE_PORT)) {
	...
	public TCPIPConnection(String remoteAddress, int remotePort,
			String localAddress) {
		if (remoteAddress.length() >= MinaConstant.MIN_LENGTH_ADDRESS) {
			this.remoteAddress = remoteAddress;
		} else {
			logger.error("Invalid remoteAddress.");
		}
		if ((remotePort >= MinaConstant.MIN_VALUE_PORT)
				&& (remotePort <= MinaConstant.MAX_VALUE_PORT)) {
			this.remotePort = remotePort;
		} else {
			logger.error("Invalid remotePort.");
		}
		if (localAddress.length() >= MinaConstant.MIN_LENGTH_ADDRESS) {
	...
	public void open() throws IOException {
		logger.debug("TCPIPConnection.open enter: address[" + remoteAddress
				+ "],[" + remotePort + "]");
			...
			if (connType == CONN_CLIENT) {
				try {
					socket = new Socket(remoteAddress, remotePort,
							InetAddress.getByName(localAddress), 0);
					initialiseIOStreams(socket);
					opened = true;
					logger.debug("opened client tcp/ip connection to "
							+ remoteAddress + " on remotePort " + remotePort);
				} catch (IOException e) {
				...
				try {
					receiverSocket = new ServerSocket(remotePort, 30,
	...
	public void close() throws IOException {
		...
		if (connType == CONN_CLIENT) {
			try {
				...
				socket = null;
				opened = false;
				logger.debug("closed client tcp/ip connection to "
						+ remoteAddress + " on remotePort " + remotePort);
			} catch (IOException e) {
			...
			try {
				...
				receiverSocket = null;
				opened = false;
				logger.debug("stopped listening tcp/ip on remotePort "
						+ remotePort);
			} catch (IOException e) {
		...
		if (connType == CONN_CLIENT) {
			...
			try {
				...
				do {
					...
					if (bytesToRead > 0) {
						logger.debug("more data (" + bytesToRead
								+ " bytes) remains in the socket");
					} else {
				...
				} while (((bytesToRead != 0) && (System.currentTimeMillis() <= endTime))
						&& (totalBytesRead < maxReceiveSize));

				logger.debug("totally read " + data.length()
						+ " bytes from socket");
			} catch (IOException e) {
				exception = e;
			}
	...
	public ByteBuffer receiveEx(int bytesToRead) throws IOException,
		...
		if (connType == CONN_SERVER) {
			...
			if (acceptedSocket != null) {
				try {
					...
					logger.error("IOException creating new client connection ",
							e);
					exception = e;
				}
			}
