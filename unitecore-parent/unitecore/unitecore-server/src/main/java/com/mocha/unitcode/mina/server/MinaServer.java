...
public class MinaServer extends IoHandlerAdapter {
	...
	private static final int BUFFER_SIZE = 2048 * 50;
	...
	public MinaServer() throws IOException {
		...
		// 与 JMX 的集成 首先获取平台提供的受控 bean 的服务器，接着创建受控
		// bean（MBean）来包装想要管理和监测的对象，这里使用的是 I/O 连接器对象。最后把创建出来的受控 bean 注册到服务器
		...
		try {
			...
			Logger.getLogger(MinaConstant.LOG_ERROR).error(
					"MalformedObjectNameException  " + e.getLocalizedMessage());
			...
			Logger.getLogger(MinaConstant.LOG_ERROR).error(
					"MBeanRegistrationException  " + e.getLocalizedMessage());
			// TODO Auto-generated catch block
			e.printStackTrace();
		} catch (NotCompliantMBeanException e) {
			Logger.getLogger(MinaConstant.LOG_ERROR).error(
					"NotCompliantMBeanException  " + e.getLocalizedMessage());
			// TODO Auto-generated catch block
			e.printStackTrace();
		}
	...
	public void exceptionCaught(IoSession session, Throwable cause) {
		if (!(cause instanceof IOException)) {
			...
			Logger.getLogger(MinaConstant.LOG_ERROR).info(
					"I/O error: " + cause.getLocalizedMessage());
		}
		session.close(true);
	}
