...
public class WeakReferenceMonitor {
	...
	private static final Log logger = LogFactory
			.getLog(WeakReferenceMonitor.class);
	...
	public static void monitor(Object handle, ReleaseListener listener) {
		if (logger.isDebugEnabled()) {
			logger.debug("Monitoring handle [" + handle
					+ "] with release listener [" + listener + "]");
		}

		// Make weak reference to this handle, so we can say when
		// handle is not used any more by polling on handleQueue.
		WeakReference<Object> weakRef = new WeakReference<Object>(handle,
				handleQueue);
	...
	private static void addEntry(Reference ref, ReleaseListener entry) {
		synchronized (WeakReferenceMonitor.class) {
			...
			if (monitoringThread == null) {
				monitoringThread = new Thread(new MonitoringProcess(),
						WeakReferenceMonitor.class.getName());
				...
				try {
					...
					if (entry != null) {
						...
						try {
							...
							logger.warn(
									"Reference release listener threw exception",
									ex);
						}
					}
