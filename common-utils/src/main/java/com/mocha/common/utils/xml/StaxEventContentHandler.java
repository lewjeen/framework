...
/**
 * SAX <code>ContentHandler</code> that transforms callback calls to
 * <code>XMLEvent</code>s and writes them to a <code>XMLEventConsumer</code>.
 * 
...
class StaxEventContentHandler extends AbstractStaxContentHandler {
	...
	/**
	 * Construct a new instance of the <code>StaxEventContentHandler</code> that
	 * writes to the given <code>XMLEventConsumer</code>. A default
	 * <code>XMLEventFactory</code> will be created.
	 * 
	 * @param consumer
	...
	/**
	 * Construct a new instance of the <code>StaxEventContentHandler</code> that
	 * uses the given event factory to create events and writes to the given
	...
	protected void startElementInternal(QName name, Attributes atts,
		...
		consumeEvent(this.eventFactory.createStartElement(name,
				attributes.iterator(),
	...
	protected void endElementInternal(QName name,
			SimpleNamespaceContext namespaceContext) throws XMLStreamException {
		List namespaces = createNamespaces(namespaceContext);
		consumeEvent(this.eventFactory.createEndElement(name,
				namespaces != null ? namespaces.iterator() : null));
	}
	...
	protected void charactersInternal(char[] ch, int start, int length)
		...
		consumeEvent(this.eventFactory.createCharacters(new String(ch, start,
				length)));
	}
	...
	protected void ignorableWhitespaceInternal(char[] ch, int start, int length)
			throws XMLStreamException {
		consumeEvent(this.eventFactory.createIgnorableSpace(new String(ch,
				start, length)));
	}
	...
	protected void processingInstructionInternal(String target, String data)
			throws XMLStreamException {
		consumeEvent(this.eventFactory
	...
	/**
	 * Create and return a list of <code>NameSpace</code> objects from the
	...
	private List<Namespace> createNamespaces(
		...
		String defaultNamespaceUri = namespaceContext
				.getNamespaceURI(XMLConstants.DEFAULT_NS_PREFIX);
		if (StringUtils.hasLength(defaultNamespaceUri)) {
			namespaces.add(this.eventFactory
					.createNamespace(defaultNamespaceUri));
		}
		for (Iterator iterator = namespaceContext.getBoundPrefixes(); iterator
				.hasNext();) {
			String prefix = (String) iterator.next();
			String namespaceUri = namespaceContext.getNamespaceURI(prefix);
			namespaces.add(this.eventFactory.createNamespace(prefix,
					namespaceUri));
		}
		return namespaces;
	}
	...
	private List<Attribute> getAttributes(Attributes attributes) {
		...
		for (int i = 0; i < attributes.getLength(); i++) {
			...
			if (!("xmlns".equals(name.getLocalPart()) || "xmlns".equals(name
					.getPrefix()))) {
				list.add(this.eventFactory.createAttribute(name,
