...
/**
 * Abstract base class for SAX <code>ContentHandler</code> implementations that
 * use StAX as a basis. All methods delegate to internal template methods,
 * capable of throwing a <code>XMLStreamException</code>. Additionally, an
...
abstract class AbstractStaxContentHandler implements ContentHandler {
	...
	public final void startDocument() throws SAXException {
		...
		try {
			...
			throw new SAXException("Could not handle startDocument: "
					+ ex.getMessage(), ex);
		}
	}
	...
	public final void startElement(String uri, String localName, String qName,
		...
		try {
			startElementInternal(toQName(uri, qName), atts,
					namespaceContextChanged ? namespaceContext : null);
			namespaceContextChanged = false;
		} catch (XMLStreamException ex) {
			throw new SAXException("Could not handle startElement: "
					+ ex.getMessage(), ex);
		}
	}

	protected abstract void startElementInternal(QName name, Attributes atts,
			SimpleNamespaceContext namespaceContext) throws XMLStreamException;

	public final void endElement(String uri, String localName, String qName)
			throws SAXException {
		try {
			endElementInternal(toQName(uri, qName),
					namespaceContextChanged ? namespaceContext : null);
			namespaceContextChanged = false;
		} catch (XMLStreamException ex) {
			throw new SAXException("Could not handle endElement: "
					+ ex.getMessage(), ex);
		}
	}
	...
	public final void characters(char ch[], int start, int length)
		...
		try {
			...
			throw new SAXException("Could not handle characters: "
					+ ex.getMessage(), ex);
		}
	}
	...
	public final void ignorableWhitespace(char[] ch, int start, int length)
		...
		try {
			...
			throw new SAXException("Could not handle ignorableWhitespace:"
					+ ex.getMessage(), ex);
		}
	}

	protected abstract void ignorableWhitespaceInternal(char[] ch, int start,
	...
	public final void processingInstruction(String target, String data)
		...
		try {
			...
			throw new SAXException("Could not handle processingInstruction: "
					+ ex.getMessage(), ex);
		}
	}
	...
	public final void skippedEntity(String name) throws SAXException {
		try {
			...
			throw new SAXException("Could not handle skippedEntity: "
					+ ex.getMessage(), ex);
		}
	}
	 ...
	 * <code>QName</code>. The qualified name can have the form
