 ...
 * points of its workflow. Note that <code>afterAccess</code> should usually be
 * called in a finally block!
 * 
 * <p>
 * The default concurrency limit of this support class is -1
 * ("unbounded concurrency"). Subclasses may override this default; check the
 * javadoc of the concrete class that you're using.
 * 
...
public abstract class ConcurrencyThrottleSupport implements Serializable {
	 ...
	 * 
	 * @return <code>true</code> if the concurrency limit for this instance is
	 *         active
		...
		if (this.concurrencyLimit > 0) {
			...
			synchronized (this.monitor) {
				...
				while (this.concurrencyCount >= this.concurrencyLimit) {
					...
					if (debug) {
						logger.debug("Concurrency count "
								+ this.concurrencyCount + " has reached limit "
				...
				if (debug) {
					logger.debug("Entering throttle at concurrency count "
		...
		if (this.concurrencyLimit >= 0) {
			synchronized (this.monitor) {
				...
				if (logger.isDebugEnabled()) {
					logger.debug("Returning from throttle at concurrency count "
							+ this.concurrencyCount);
				}
				this.monitor.notify();
			}
