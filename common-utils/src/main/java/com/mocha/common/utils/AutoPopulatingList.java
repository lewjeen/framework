...
public class AutoPopulatingList<E> implements List<E>, Serializable {
	...
	/**
	 * Creates a new <code>AutoPopulatingList</code> that is backed by a
	 * standard {@link ArrayList} and adds new instances of the supplied
	 * {@link Class element Class} to the backing {@link List} on demand.
	 */
	...
	/**
	 * Creates a new <code>AutoPopulatingList</code> that is backed by the
	 * supplied {@link List} and adds new instances of the supplied
	 * {@link Class element Class} to the backing {@link List} on demand.
	 */
	...
	/**
	 * Creates a new <code>AutoPopulatingList</code> that is backed by a
	 * standard {@link ArrayList} and creates new elements on demand using the
	 * supplied {@link ElementFactory}.
	 */
	...
	/**
	 * Creates a new <code>AutoPopulatingList</code> that is backed by the
	 * supplied {@link List} and creates new elements on demand using the
	 * supplied {@link ElementFactory}.
	 */
	public AutoPopulatingList(List<E> backingList,
	 ...
	 * <code>Class.newInstance()</code> on a given element class.
	 * 
	 * @see java.lang.Class#newInstance()
	 */
	private static class ReflectiveElementFactory<E> implements
			ElementFactory<E>, Serializable {
		...
		public ReflectiveElementFactory(Class<? extends E> elementClass) {
			...
			Assert.isTrue(!elementClass.isInterface(),
					"Element class must not be an interface type");
		...
		public E createElement(int index) {
			try {
				...
				throw new ElementInstantiationException(
						"Unable to instantiate element class ["
								+ this.elementClass.getName()
								+ "]. Root cause is " + ex);
			} catch (IllegalAccessException ex) {
				throw new ElementInstantiationException(
						"Cannot access element class ["
								+ this.elementClass.getName()
								+ "]. Root cause is " + ex);
			}
		}
