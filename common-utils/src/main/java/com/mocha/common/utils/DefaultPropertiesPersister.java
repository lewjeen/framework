...
package com.mocha.common.utils;
 ...
 * the native parsing of <code>java.util.Properties</code>.
 * 
 * <p>
 ...
 * <code>java.util.Properties</code> unfortunately lacks up until JDK 1.5: You
 * can only load files using the ISO-8859-1 charset there.
 * 
 * <p>
 * Loading from and storing to a stream delegates to
 * <code>Properties.load</code> and <code>Properties.store</code>, respectively,
 * to be fully compatible with the Unicode conversion as implemented by the JDK
 * Properties class. On JDK 1.6, <code>Properties.load/store</code> will also be
 * used for readers/writers, effectively turning this class into a plain
 ...
 * through the <code>loadFromXml</code> and <code>storeToXml</code> methods. The
 * default implementations delegate to JDK 1.5's corresponding methods, throwing
 * an exception if running on an older JDK. Those implementations could be
 * subclassed to apply custom XML handling on JDK 1.4, for example.
 * 
...
public class DefaultPropertiesPersister implements PropertiesPersister {
	...
	private static final boolean loadFromReaderAvailable = ClassUtils
			.hasMethod(Properties.class, "load", new Class[] { Reader.class });

	// Determine whether Properties.store(Writer, String) is available (on JDK
	// 1.6+)
	private static final boolean storeToWriterAvailable = ClassUtils.hasMethod(
			Properties.class, "store",
	...
	public void storeToXml(Properties props, OutputStream os, String header)
		...
		try {
			...
			throw new IOException(
					"Cannot store properties XML file - not running on JDK 1.5+: "
							+ err.getMessage());
		}
	}
