...
import com.mocha.common.utils.Assert;
import com.mocha.common.utils.StringUtils;

/**
 * SAX <code>XMLReader</code> that reads from a StAX
 * <code>XMLStreamReader</code>. Reads from an <code>XMLStreamReader</code>, and
...
class StaxStreamXMLReader extends AbstractStaxXMLReader {
	...
	/**
	 * Constructs a new instance of the <code>StaxStreamXmlReader</code> that
	 * reads from the given <code>XMLStreamReader</code>. The supplied stream
		...
		if (getContentHandler() != null) {
			...
			if (hasNamespacesFeature()) {
				for (int i = 0; i < reader.getNamespaceCount(); i++) {
					startPrefixMapping(reader.getNamespacePrefix(i),
				...
				getContentHandler().startElement(qName.getNamespaceURI(),
						qName.getLocalPart(), toQualifiedName(qName),
						getAttributes());
			} else {
	...
	private void handleEndElement() throws SAXException {
		if (getContentHandler() != null) {
			QName qName = reader.getName();
			if (hasNamespacesFeature()) {
				getContentHandler().endElement(qName.getNamespaceURI(),
						qName.getLocalPart(), toQualifiedName(qName));
				for (int i = 0; i < reader.getNamespaceCount(); i++) {
	...
	private void handleCharacters() throws SAXException {
		if (getContentHandler() != null && reader.isWhiteSpace()) {
			getContentHandler().ignorableWhitespace(reader.getTextCharacters(),
					reader.getTextStart(), reader.getTextLength());
			return;
		}
		if (XMLStreamConstants.CDATA == reader.getEventType()
				&& getLexicalHandler() != null) {
			getLexicalHandler().startCDATA();
		}
		if (getContentHandler() != null) {
			getContentHandler().characters(reader.getTextCharacters(),
					reader.getTextStart(), reader.getTextLength());
		}
		if (XMLStreamConstants.CDATA == reader.getEventType()
				&& getLexicalHandler() != null) {
	...
	private void handleComment() throws SAXException {
		if (getLexicalHandler() != null) {
			getLexicalHandler().comment(reader.getTextCharacters(),
					reader.getTextStart(), reader.getTextLength());
		}
	}
	...
	private void handleDtd() throws SAXException {
		if (getLexicalHandler() != null) {
			...
			getLexicalHandler().startDTD(null, location.getPublicId(),
	...
	private void handleProcessingInstruction() throws SAXException {
		if (getContentHandler() != null) {
			getContentHandler().processingInstruction(reader.getPITarget(),
					reader.getPIData());
		}
	}
	...
	private Attributes getAttributes() {
		...
		for (int i = 0; i < reader.getAttributeCount(); i++) {
			...
			attributes.addAttribute(namespace, reader.getAttributeLocalName(i),
					toQualifiedName(reader.getAttributeName(i)), type,
