 ...
 * {@link org.springframework.beans.factory.config.MethodInvokingFactoryBean}
 * and
...
public class MethodInvoker {
	...
	public void prepare() throws ClassNotFoundException, NoSuchMethodException {
		if (this.staticMethod != null) {
			...
			if (lastDotIndex == -1
					|| lastDotIndex == this.staticMethod.length()) {
		...
		if (targetMethod == null) {
			throw new IllegalArgumentException(
					"Property 'targetMethod' is required");
		}
	...
	protected Method findMatchingMethod() {
		...
		Method[] candidates = ReflectionUtils
				.getAllDeclaredMethods(getTargetClass());
	...
	public Object invoke() throws InvocationTargetException,
		...
		if (targetObject == null
				&& !Modifier.isStatic(preparedMethod.getModifiers())) {
			throw new IllegalArgumentException(
