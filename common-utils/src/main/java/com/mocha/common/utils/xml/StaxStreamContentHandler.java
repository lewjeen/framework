...
class StaxStreamContentHandler extends AbstractStaxContentHandler {
	...
	/**
	 * Constructs a new instance of the <code>StaxStreamContentHandler</code>
	 * that writes to the given <code>XMLStreamWriter</code>.
	 * 
	 * @param streamWriter
	...
	protected void startElementInternal(QName name, Attributes attributes,
		...
		if (namespaceContext != null) {
			...
			for (Iterator<String> iterator = namespaceContext
					.getBoundPrefixes(); iterator.hasNext();) {
				String prefix = iterator.next();
				streamWriter.writeNamespace(prefix,
						namespaceContext.getNamespaceURI(prefix));
				streamWriter.setPrefix(prefix,
						namespaceContext.getNamespaceURI(prefix));
			}
		}
		for (int i = 0; i < attributes.getLength(); i++) {
			QName attrName = toQName(attributes.getURI(i),
					attributes.getQName(i));
			if (!("xmlns".equals(attrName.getLocalPart()) || "xmlns"
					.equals(attrName.getPrefix()))) {
				streamWriter.writeAttribute(attrName.getPrefix(),
