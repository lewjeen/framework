...
public class Receiver extends ProcessingThread {
	...
	final public ByteBuffer receivePDUFromConnectionEx(Connection connection)
	...
	final protected ByteBuffer receivePDUFromConnection(Connection connection)
		...
		logger.debug("has unprocessed bytes: "
		...
		if (pduBuffer == null) { // only if we didn't manage to get pdu from
									// unproc
			...
			if ((buffer != null) && (buffer.length() != 0)) {
				...
				if ((unprocessed.getUnprocessed().length() > 0)
						&& ((unprocessed.getLastTimeReceived() + timeout) < System
								.currentTimeMillis())) {
					logger.debug("and it's been very long time.");
					unprocessed.reset();
					throw new TimeoutException(timeout,
							unprocessed.getExpected(), unprocessed
									.getUnprocessed().length());
				}
			}
		}
		logger.debug("has unprocessed bytes: "
	...
	private boolean canContinueReceiving(long startTime, long timeout) {
		return timeout == MinaConstant.RECEIVE_BLOCKING ? true : System
